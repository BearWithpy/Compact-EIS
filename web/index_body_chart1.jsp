...
<head>
    ...
    <script type="text/javascript">
      ...
      //        out.println("</script>");
        ...
        saleOrderData2= saleOrderData.splice(1,6);
        rMateChartH5.create("chart5", "body-content1-chart", "", "95%", "95%");

        //스트링 형식으로 레이아웃 정의.
        var layoutStr5 =
            '<rMateChart backgroundColor="" borderStyle="none">'
            +'<Options>'
            +'<Caption text=""/>'
            ...
            +'<verticalAxis>'
            // +'<LinearAxis/>'
            +'<BrokenAxis id="vAxis" brokenMinimum="45000" brokenMaximum="100000" brokenRatio="0.8" interval="10000" formatter="{numfmt}" />'
            +'</verticalAxis>'
...
<body>
    <div id="body-content1" style=" position: relative; " >
        <div class="bt-group-chart1">
            <div style="margin-top:-15px;">
                <p style="margin-top: 20px">수주 현황</p>
            </div>
            <hr style="margin: 12px; color:grey; margin-top:-20px;border-width:2px 0 0 0;">
            <div style="position:absolute; right:5px; top:0px;">
                <button type="button" id="minusbutton1" style="border:none; font-size:27px;background-color:white; color:grey;">-</button>
                <button type="button" id="xbutton1" style="border:none; font-size:20px; background-color:white; color:grey;">X</button>
            </div>
        </div>
        <div id="body-content1-chart" style="width:100%;height:300px; margin-bottom:10px;"></div>
    </div>
</body>
<script>
    function chartItemClickHandler(seriesId, displayText, index, data, values){
        var chUrl = "http://www.localhost:8080/index_body_chart1.jsp?";
        window.open(chUrl, '_blank');
    }

    var count1 =0;
    document.getElementById("minusbutton1").addEventListener('click',minusbutton6);
    function minusbutton6(){
        console.log("축소버튼클릭!");
        if(count1%2==0){
            const element = document.getElementById('body-content1-chart');
            element.style.cssText  = 'visibility:hidden; transition: all 0.3s;  height: 0vh;  opacity:0;';
            const button = document.getElementById('minusbutton1');
            button.innerText = '+';
            count1++;
        } else if(count1%2==1){
            const element = document.getElementById('body-content1-chart');
    ...
    document.getElementById("xbutton1").addEventListener('click',xbutton6);
    function xbutton6(){
        console.log("X버튼클릭!");
        const element = document.getElementById('body-content1');

        element.style.cssText  = 'visibility:hidden; transition: all 0.3s;  height: 0vh; opacity:0;';
    }
    document.getElementById("detailbutton1").addEventListener('click',detailbutton6);
